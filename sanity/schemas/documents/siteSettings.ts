...
export const siteSettings = defineType({
  ...
  fields: [
    ...
    defineField({
      ...
      description: 'Background image for single image mode (used when slideshow is disabled)',
    }),
    defineField({
      name: 'heroSlideshowEnabled',
      title: 'Enable Hero Slideshow',
      type: 'boolean',
      description: 'Toggle between single image and slideshow mode',
      initialValue: false,
    }),
    defineField({
      name: 'heroMedia',
      title: 'Hero Slideshow Media',
      type: 'array',
      of: [{ type: 'heroMediaItem' }],
      description: 'Add images and/or videos for the hero slideshow',
      options: {
        sortable: true,
      },
      hidden: ({ document }) => !document?.heroSlideshowEnabled,
      validation: (Rule) => Rule.max(10).warning('Consider limiting to 10 items for performance'),
    }),
    defineField({
